...
def sign(bob_private_key, encrypted_message):
    d, n = bob_private_key
    signed_message = [(pow(char, d, n)) for char in encrypted_message]
    return signed_message

def verify(bob_public_key, signed_message):
    e, n = bob_public_key
    verified_message = [(pow(char, e, n)) for char in signed_message]
...
def main():
    ...
    signed_message = sign(bob_private_key, encrypted_message)
    print("Message after being signed by Bob: ", signed_message)

    verified_message = verify(bob_public_key, signed_message)

    if verified_message == encrypted_message:
         print("Signature verified. The message is sent by Bob. ", verified_message)
    else:
         print("Signature verification failed. Message is not sent by Bob.")

    decrypted_message = decrypt(alice_private_key, verified_message)
