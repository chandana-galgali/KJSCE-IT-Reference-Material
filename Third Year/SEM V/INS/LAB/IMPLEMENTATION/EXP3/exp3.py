...
def main():
    ...
    if len(plaintext) % 8 != 0 or not all(c in '01' for c in plaintext):
        print("Invalid plaintext! It must be a binary string and a multiple of 8 bits.")
        return
    ...
    decrypted_bits = xor_bitstrings(ciphertext_bits, cipher.generate_keystream(len(ciphertext_bits)))
